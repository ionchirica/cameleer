...
let get_acc_from_header (spec : Uast.val_spec) (pty : Uast.pty) =
  ...
  let args =
    List.map
      (fun arg ->
        let id =
          match (arg : Uast.labelled_arg) with
          | Lnone pid -> pid
          | Loptional pid -> pid
          | Lnamed pid -> pid
          | Lghost (pid, _) -> pid
...
let mk_cursor info (sval : Uast.s_val_description) =
  ...
  let use_cursor =
    O.mk_odecl Loc.dummy_position
      (Duseimport
         (Loc.dummy_position, false, [ (seq_lib, None); (cursor_lib, None) ]))
  in
  ...
  let create_param =
    (Loc.dummy_position, Some (T.mk_id "collection"), false, Uterm.pty structure)
  in
  let result_pat =
    { pat_desc = Pvar (T.mk_id "result"); pat_loc = Loc.dummy_position }
  in
    ...
    T.mk_term
      (Tinfix
         ( T.mk_term
             (Tidapp
                ( Qident (T.mk_id f),
                  [ T.mk_term (Tident (Qident (T.mk_id "result"))) ] )),
           T.mk_id "infix =",
           t ))
  in
  ...
  let create_post =
    [
      ...
      ( Loc.dummy_position,
        [ (result, res "permitted" (T.term false permitted)) ] );
  ...
  let create =
    O.mk_odecl Loc.dummy_position
      (Dlet (T.mk_id "create", false, Expr.RKnone, create_expr))
  in
  let clone =
    O.mk_odecl Loc.dummy_position
      (Dcloneexport (Loc.dummy_position, cursor_lib, []))
  in
  O.mk_omodule Loc.dummy_position
    (T.mk_id (String.capitalize_ascii sval.vname.txt))
    [ use_cursor; create; clone ]
...
let s_structure, s_signature =
  ...
  and s_signature_item_desc info loc sig_item_desc =
    ...
    | Sig_val ({ vname = x; _ } as s_val)
      when (String.starts_with ~prefix:"iter" x.txt
           || String.starts_with ~prefix:"fold" x.txt
           || String.starts_with ~prefix:"map" x.txt
           || String.starts_with ~prefix:"filter" x.txt)
